'use client';

import { useState } from 'react';
import { supabase } from '@/lib/supabase';
import { Shield, KeyRound, ArrowRight } from 'lucide-react';

export default function LoginPage() {
  const [isStaffOrGM, setIsStaffOrGM] = useState(false);
  const [passkey, setPasskey] = useState('');
  const [errorMsg, setErrorMsg] = useState('');

  // Handle Login Provider (Google / Discord)
  const handleOAuthLogin = async (provider) => {
    // Jika memilih sebagai Staff/GM, wajib isi Passkey Pengurus
    if (isStaffOrGM && !passkey) {
      setErrorMsg('Masukkan Master Passkey Pengurus terlebih dahulu!');
      return;
    }

    // Nanti di sini kita verifikasi Passkey via Supabase RPC / App Settings
    if (isStaffOrGM) {
      // Contoh simpan status passkey sementara di Session Storage
      sessionStorage.setItem('guild_master_passkey', passkey);
    }

    const { error } = await supabase.auth.signInWithOAuth({
      provider: provider,
      options: {
        redirectTo: `${window.location.origin}/auth/callback`,
      },
    });

    if (error) {
      setErrorMsg(error.message);
    }
  };

  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl space-y-6">
        
        {/* Header Header */}
        <div className="text-center space-y-2">
          <div className="inline-flex p-3 bg-indigo-500/10 text-indigo-400 rounded-xl mb-2">
            <Shield className="w-8 h-8" />
          </div>
          <h1 className="text-2xl font-bold tracking-wide">Guild Portal</h1>
          <p className="text-slate-400 text-sm">Masuk atau daftar untuk mengakses sistem Guild</p>
        </div>

        {/* Form Error Message */}
        {errorMsg && (
          <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-3 rounded-xl text-xs text-center">
            {errorMsg}
          </div>
        )}

        {/* Checkbox Login Akses Staff / GM */}
        <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-800 space-y-3">
          <label className="flex items-center space-x-3 cursor-pointer">
            <input
              type="checkbox"
              checked={isStaffOrGM}
              onChange={(e) => {
                setIsStaffOrGM(e.target.checked);
                setErrorMsg('');
              }}
              className="w-4 h-4 rounded border-slate-700 bg-slate-900 text-indigo-500 focus:ring-indigo-500 focus:ring-offset-slate-900"
            />
            <span className="text-xs font-semibold text-slate-300">Saya adalah Staff / Guild Master</span>
          </label>

          {/* Input Single Passkey Pengurus */}
          {isStaffOrGM && (
            <div className="space-y-1.5 pt-2">
              <label className="text-xs text-slate-400 flex items-center gap-1.5">
                <KeyRound className="w-3.5 h-3.5 text-amber-400" />
                Master Passkey Pengurus
              </label>
              <input
                type="password"
                placeholder="Masukkan Passkey..."
                value={passkey}
                onChange={(e) => setPasskey(e.target.value)}
                className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500"
              />
            </div>
          )}
        </div>

        {/* Tombol Login OAuth */}
        <div className="space-y-3 pt-2">
          <button
            onClick={() => handleOAuthLogin('google')}
            className="w-full flex items-center justify-center gap-3 bg-white text-slate-900 font-semibold py-2.5 px-4 rounded-xl hover:bg-slate-100 transition-colors text-sm"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z" />
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" />
            </svg>
            Lanjutkan dengan Google
          </button>

          <button
            onClick={() => handleOAuthLogin('discord')}
            className="w-full flex items-center justify-center gap-3 bg-[#5865F2] text-white font-semibold py-2.5 px-4 rounded-xl hover:bg-[#4752C4] transition-colors text-sm"
          >
            <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
              <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.061 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.028z" />
            </svg>
            Lanjutkan dengan Discord
          </button>
        </div>

        <div className="text-center pt-2">
          <p className="text-xs text-slate-500">
            Pendaftaran baru otomatis berstatus <span className="text-slate-400 font-medium">Guest/Pending</span> sampai disetujui Staff.
          </p>
        </div>

      </div>
    </div>
  );
}